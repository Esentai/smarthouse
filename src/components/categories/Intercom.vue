<template>
  <div class="container">
    <div class="content">
      <b-button class="addBtn" variant="primary" v-b-modal.modal-prevent-closing>Add new intercom device</b-button>
      <b-modal id="modal-prevent-closing" ref="modal" title="Add new intercom device" @show="resetModal" @hidden="resetModal" @ok="handleOk">
      ...
      <h1 class="title">Intercom</h1>
      <div class="item" v-for="device in devices" :key="device.deviceID">
        <h3 class="itemText">{{device.name}}</h3>
        <b-button class="itemBtn" variant="primary" @click="watchBtn" >open door</b-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    watchBtn(){
      this.$alert(`Error when opening the door!`);
    }
